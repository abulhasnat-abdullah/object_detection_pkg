...
class ObjectDetectionNode(Node):
    def __init__(self):
        ...
        self.model = YOLO("yolov8x.pt")  # Or use a smaller model like yolov8n.pt
        self.device = "cuda" if self.model.device.type == "cuda" else "cpu"

        # Set up image save directory and CSV logging
        ...
        # Cooldown management to avoid duplicate logs
        self.last_seen = {}
    ...
    def listener_callback(self, msg):
        ...
        if results.boxes is None or len(results.boxes.cls) == 0:
            # Show raw frame if no detection
        ...
        for box in results.boxes:
            ...
            if not last_time or now - last_time >= self.cooldown:
                ...
                labels_to_save.append((label, conf))
                save_needed = True

        # Save annotated frame and log detection
        if save_needed:
        ...
        # Always show frame with detections
        cv2.imshow("YOLOv8 Detection Feed", frame)
        cv2.waitKey(1)
